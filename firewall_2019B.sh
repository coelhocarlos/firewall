...
# Server IP
SERVER_IP="$(ip addr show enp2s0 | grep 'inet ' | cut -f2 | awk '{ print $2}')"
...
do
	...
	echo "Allow connection ALternative to '$ip' on port 2121"
...
$IPT -A INPUT  -p tcp -m multiport --dports 21,2121,80,443,8081,8082,8083,8084 -m state --state NEW,ESTABLISHED -j ACCEPT
$IPT -A OUTPUT -p tcp -m multiport --dports 21,2121,80,443,8081,8082,8083,8084 -m state --state ESTABLISHED     -j ACCEPT
